...
CARTELLA_VIDEO_INPUT = "Video/"        # La cartella che contiene le sottocartelle delle date
...
def processa_riga_specifica():
    ...
    # Estraiamo i primi 10 caratteri per la cartella (es. "04-03-2025")
    cartella_data = nome_file_excel[:10]
    
    # Percorso: Video/04-03-2025/nome_file.mp4
    ...
    # Calcolo Frame per labels.csv
    off = t_ini - start
    f_ini = int(off * fps)
    f_fine = f_ini + int((t_fine - t_ini) * fps)

    # --- SCRITTURA LABEL (CORRETTA) ---
    file_label_path = os.path.join(CARTELLA_OUTPUT, "mappa_labels.csv")
    file_exists = os.path.isfile(file_label_path)
    
    # Usiamo newline='' per evitare problemi di righe fantasma o mancate
    with open(file_label_path, mode='a', newline='', encoding='utf-8') as f:
        # Usiamo il punto e virgola come separatore per una compatibilità perfetta con Excel
    ...
    # Opzionale: apre il video subito su Mac per controllo
...
if __name__ == "__main__":
    ...
    #file_label_path = os.path.join(CARTELLA_OUTPUT, "mappa_labels.csv")
    #file_exists = os.path.isfile(file_label_path)
    #with open(file_label_path, mode='a', newline='') as f:
    #    writer = csv.writer(f)
    #    if not file_exists:
    #        writer.writerow(['clip_name', 'topo', 'fps', 'f_inizio', 'f_fine', 'f_tot'])
    #    writer.writerow([output_name, id_topo, round(fps, 2), f_ini, f_fine, int(durata * fps)])
